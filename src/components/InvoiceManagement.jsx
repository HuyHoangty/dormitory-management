...
function submitForm() {
    ...
    const handleStatusChange = (index) => {
        ...
        const currentStatus = newInvoices[index].status;
        if (currentStatus === 'Đã thanh toán') {
            newInvoices[index].status = 'Chưa thanh toán';
        } else {
            newInvoices[index].status = 'Đã thanh toán';
        }
    ...
    return (
      ...
      <h1 className="text-2xl font-bold mb-6">Quản lý thanh toán</h1>
                    <div className="flex mb-6">
                        {['Tất cả', 'Chưa thanh toán', 'Đã thanh toán'].map((tab) => (
                            <button
                                key={tab}
                                    ...
                                    <tr key={index} className="border-t">
                                        ...
                                        <td className="py-8 px-4">
                                            <button
                                                ...
                                                className={`px-2 py-1 rounded ${item.status === 'Đã thanh toán' ? 'bg-green-100 text-green-700' : item.status === 'Chưa thanh toán' ? 'bg-red-100 text-red-700' : 'bg-yellow-100 text-yellow-700'}`}
                                            >
                                                {item.status}
                                            </button>
                                        </td>
